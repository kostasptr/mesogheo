...
import ArrowLeft from "./_ArrowLeft";
import ArrowRight from "./_ArrowRight";
  ...
  return (
    <div className="col-span-full sm2:relative" id={isHome ? "rooms" : ''}>
      <div className="navigation-wrapper text-92_green text-opacity-60">
        <div ref={sliderRef} className="keen-slider w-full">
          {imagesArray.map((images, idx) => (
            ...
            >
              ...
              { title
                  ? (<figcaption className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 font-serif font-medium text-t28 md:text-f3.25 lg:text-4.25xl sm2:text-f3.625 sm3:text-f5 tracking-xlwidest md:tracking-sp20 lg:tracking-sp24 sm2:tracking-sp32 sm3:tracking-sp36 leading-tight md:leading-height67 lg:leading-height44 sm2:leading-height60 ${images[0].shadow ? 'filter drop-shadow-0.5' : ''}`}>{images[0]?.title || title}</figcaption>)
                  : (
        ...
        {slider && (
          <div className="flex justify-center space-x-0.5 md:space-x-1 lg:space-x-2 absolute z-10 bottom-2 -inset-x-full h-2.5 md:h-3 lg:h-4">
